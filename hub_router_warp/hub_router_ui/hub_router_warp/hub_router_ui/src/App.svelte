<script lang="ts">
  ...
  function compute_slots(hub: HubInfo): [string, number, number][] {
    ...
    for (let s of slots) {
      ...
      if (browser_util[browser] === undefined) browser_util[browser] = [0, 0];
    ...
    flat.sort((a, b) => b[1] - a[1] || b[2] - a[2] || b[0].localeCompare(a[0]));
    return flat;
  }
      ...
      <div class="hubs-row">
        {#each sort_hubs($api_data.data) as hub_data}
          ...
          <div
            ...
            ) % 360}, {(Math.abs(hashCode(hub_data.router_hub_state.name)) %
              20) +
              50}%, 72%)"
          >
            <div
